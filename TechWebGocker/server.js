...
app.get("/primaPagina/ottieniStorie", function(req,res){
	fs.readFile(__dirname + "/storie.json",function(err,storie) {
		if (err){
			console.log(err);
...
app.get("/player/ottienistoria", function(req,res){
	fs.readFile(__dirname + "/" + storiaDaCaricare + ".json",function(err,storia) {
		if (err){
			console.log(err);
			res.status(500).send('Internal server error');
		}
...
io.on('connection', function (socket) {
	...
	//gestisce le risposte con metodo server black-box
...
app.get('/autore/saveStory', function (req, res) {
	console.log("Ricevuto richiesta salva storia");
	var storyID = parseInt(req.query.id);
	var stories = openAndParseJsonFile("storie.json");
	var story = stories.storie.find(s => s.id === storyID);
	var storyName = story.nome;
	...
	res.setHeader('Content-Type', 'application/json; charset=UTF-8');
	res.end(JSON.stringify(singleStory,null,4));
});
