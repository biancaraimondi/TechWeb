...
function saveStory(){
	...
	xhr.onreadystatechange = () => {
		if (xhr.readyState === 4 && xhr.status === 200){
			var strSaveStory = xhr.responseText;
			var nameStory = JSON.parse(strSaveStory).nomestoria;
			var data = new Blob([strSaveStory], {type: 'application/json'});
			var url = URL.createObjectURL(data);
			var link = document.createElement('a');
			link.href = url;
			link.download = nameStory + ".json";
	...
	if(indexOfCheckedRadio!==-1){
		...
		document.getElementById("formattivita").hidden = true;
		document.getElementById("messageerrorfunction").innerHTML = "";
		xhr.open("GET", '/autore/saveStory?id='+indexOfCheckedRadio, true);
		xhr.send();
	}
	else {
		document.getElementById("messageerrorfunction").innerHTML = ('<i>&nbspNon è stata selezionata nessuna storia archiviata.</i>');
